<template>
  <div>
    <div class="customerInfoCard">
      ...
      <div class="btnWarp">
        <!-- <el-button
          ...
          >添加联系人</el-button
        > -->
        <el-button plain icon="el-icon-edit" size="small" v-btnPermi="['ghkh_new_follow_up']"
          @click="handleAction('2')">写新跟进</el-button>
        <!-- <el-button
          plain
          ...
          >新建任务</el-button
        > -->
        <el-button plain icon="el-icon-refresh-left" size="small" v-btnPermi="['ghkh_transfer_customer']"
          @click="handleAction('3')">转移客户</el-button>
        <!-- <el-button
          plain
          ...
          >移入公海</el-button
        > -->
        <el-button plain icon="el-icon-date" size="small" v-btnPermi="['ghkh_add_collaboration']"
          @click="handleAction('6')">添加协作</el-button>
        <!-- <el-button
        ...
        <!-- <el-button
          ...
          >重点关注</el-button
        > -->
        <el-button plain icon="el-icon-edit-outline" size="small" @click="edit()" v-if="customerInfo.status == 0"
          v-btnPermi="['ghkh_edit_customer']">编辑客户</el-button>
        <el-button v-else icon="el-icon-edit-outline" size="small" @click="editsea()"
          v-btnPermi="['ghkh_edit_customer']">编辑客户</el-button>
        <el-button plain icon="el-icon-circle-close" size="small" v-btnPermi="['ghkh_delete_customer']"
          @click="delcustomer('2')">删除客户</el-button>
      </div>
    </div>

    <div class="pd">
      <el-tabs v-model="activeName" @tab-click="handleClick" class="tabsSelf">
        <el-tab-pane v-if="istabShow('ghkh_general_info')" label="概况信息" name="1">
          <div>
            <div>
              <div class="titleSelf" style="display: flex; justify-content: space-between">
                <div><i class="el-icon-s-flag"></i> 基本信息</div>
                <div @click="activeName = '2'" style="font-weight: 500; cursor: pointer">
              ...
              <div class="titleSelf" style="display: flex; justify-content: space-between">
                ...
                <div @click="JumpContactdetail(customerInfo.contactEntity.id)"
                ...
                <div class="box">
                  <div class="each">
                    ...
                    <div style="
                      ...
                      <p style="padding: 0; margin: 0">
                        <span class="action_span" @click="handledelEdit(customerInfo.contactEntity.id)"><i
                            class="el-icon-edit"></i>编辑</span>
                        <span class="action_span" @click="
            ...
            <div class="titleSelf" style="display: flex; justify-content: space-between">
              ...
              <div style="font-weight: 500; cursor: pointer">
                ...
                <el-button type="primary" icon="el-icon-edit" size="mini" style="margin-left: 10px"
                  v-btnPermi="['kh_new_follow_up']" @click="handleAction('2')">写跟进</el-button>
              </div>
            </div>
            <div class="timelineBox">
              <el-timeline style="padding-left: 5px">
                <el-timeline-item v-for="(item, index) in dynamicList" :key="index" color="#0079fe"
                  :timestamp="item.date" placement="top">
                  <el-card style="margin-top: 10px; background: #f7fbfe" v-for="(items, indexs) in item.list"
                    :key="indexs">
                    ...
                    <div class="commentList" v-for="(common, commonindex) in items.list" :key="commonindex">
                    ...
                    <div v-if="items.type == 1">
                      <el-button plain v-btnPermi="['kh_follow_up_details']" icon="el-icon-date" size="small"
                        @click="detail(items.id)">详情</el-button>
                      <el-button plain icon="el-icon-date" size="small" @click="comment(items)">评论</el-button>
                    </div>
                    <div class="commentDiv" v-if="items.id == currentCommentId">
                      <el-input placeholder="请输入内容" v-model="comments">
                      </el-input>
                      <el-button plain icon="el-icon-date" size="small" @click.native="SendComment(items)">评论
                      </el-button>
                    </div>
                  </el-card>
                </el-timeline-item>
        ...
        <el-tab-pane v-if="istabShow('ghkh_data_details')" label="资料详情" name="2">
          <div>
            <div class="titleSelf" style="display: flex; justify-content: space-between">
              <div><i class="el-icon-s-flag"></i> 基本信息</div>
              <el-button type="primary" icon="el-icon-edit-outline" size="small" @click="editsea()"
                v-btnPermi="['ghkh_edit_customer']">编辑客户</el-button>
            </div>
            <div class="tableSelf">
              <div class="box">
            ...
            <div class="tableSelf">
              <div class="box">
                ...
                <div class="each">
                  <div>下级客户：</div>
                  <div>{{  customerInfo.childCustomerNum  }}</div>
                </div>
              </div>
              <div class="box">
                <div class="each">
                ...
                <div class="each">
                  <div>备注信息：</div>
                  <div>{{  customerInfo.contactEntity.remark  }}</div>
                </div>
              </div>
            </div>
        ...
        <el-tab-pane v-if="istabShow('ghkh_contact')" label="联系人" name="3">
          <div style="
              margin-bottom: 10px;
              display: flex;
              justify-content: flex-end;
            ">
            <el-button style="margin-right: 10px" type="primary" icon="el-icon-plus" size="small"
              @click="handleAddConcant" v-btnPermi="['contacts_add']">添加</el-button>
            ...
            <el-button plain v-btnPermi="['contacts_delete']" icon="el-icon-switch-button" size="small"
              @click="handledelConcant()">删除</el-button>
          </div>
          <el-table ref="multipleTable" :data="concantTableData" tooltip-effect="dark" style="width: 100%"
            ...
            <el-table-column prop="action" label="操作" width="180">
              <template slot-scope="scope">
                <el-button style="margin-right: 10px" @click.native.prevent="JumpContactdetail(scope.row.id)"
                  size="mini" type="primary" plain>
                  详情
                </el-button>
                <el-popconfirm title="确定删除吗？" @confirm="delContant(scope.row.id)">
        ...
        <el-tab-pane v-if="istabShow('ghkh_lower_level_customers')" label="下级客户" name="4">
          <div style="
              margin-bottom: 10px;
              display: flex;
              justify-content: flex-end;
            ...
            <el-dropdown trigger="click" v-btnPermi="['contacts_export']" style="margin-right: 10px">
              <el-button type="primary" icon="el-icon-upload" size="small">导出</el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item icon="el-icon-circle-plus" @click.native="nextcustomerExport">选中导出</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </div>
          <el-table ref="multipleTable" :data="nextcustomerTableData" tooltip-effect="dark" style="width: 100%"
            ...
            <el-table-column prop="state" label="客户状态" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="contactName" label="首联系人" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="contactPhone" label="手机号码" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="userId" label="客户归属" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="remark" label="备注" show-overflow-tooltip>
            </el-table-column>
          </el-table>
        </el-tab-pane>
        <el-tab-pane v-if="istabShow('ghkh_follow_up_record')" label="跟进记录" name="5">
          <div class="titleSelf"><i class="el-icon-s-flag"></i> 跟进记录</div>
          <div style="text-align: center; padding-top: 30px; color: #999999" v-if="followRecordData.length == 0">
            暂无数据
          </div>
          <div class="timelineBox">
            <el-timeline style="padding-left: 5px">
              <el-timeline-item v-for="(item, index) in followRecordData" :key="index" color="#0079fe"
                :timestamp="item.date" placement="top">
                <el-card style="margin-top: 10px; background: #f7fbfe" v-for="(items, indexs) in item.list"
                  :key="indexs">
                  <h4 class="dynamicUserInfoDiv">
                  ...
                  <div v-if="items.type == 1">
                    <el-button plain icon="el-icon-date" size="small" v-btnPermi="['kh_follow_up_details']"
                      @click="detail(items.id)">详情</el-button>
                    <el-button plain icon="el-icon-date" size="small" @click="comment(items)">评论</el-button>
                  </div>
                  <div class="commentDiv" v-if="items.id == currentCommentId">
                    ...
                    <el-button plain icon="el-icon-date" size="small" @click.native="SendComment(items)">评论</el-button>
                  </div>
                </el-card>
              </el-timeline-item>
            </el-timeline>
        ...
        <el-tab-pane v-if="istabShow('ghkh_mission_record')" label="任务记录" name="6">
          <div class="titleSelf"><i class="el-icon-s-flag"></i> 任务记录</div>
          <div style="text-align: center; padding-top: 30px; color: #999999" v-if="workRecordData.length == 0">
            暂无数据
          </div>
          <div class="TaskCard" v-for="(item, index) in workRecordData" :key="index">
            ...
            <div style="width: 100px">
              <el-button plain icon="el-icon-date" size="small" @click="taskDetailDialog(item.id)">详情</el-button>
            </div>
          </div>
        </el-tab-pane>
        <el-tab-pane v-if="istabShow('ghkh_work_order_record')" label="工单记录" name="7">
          <el-table ref="multipleTable" :data="workOrderRecordData" tooltip-effect="dark" style="width: 100%"
            @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55"> </el-table-column>
            <el-table-column prop="workId" label="工单编号"> </el-table-column>
            <el-table-column prop="workTitle" label="工单标题">
            </el-table-column>
            <el-table-column prop="createBy" label="发起人员" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="operationStaff" label="处理人员" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="createTime" label="发起时间" show-overflow-tooltip>
            </el-table-column>
            ...
            <el-table-column prop="remark" label="评论记录"> </el-table-column>
            <el-table-column prop="userId" label="操作">
              <template slot-scope="scope">
                <el-button style="margin-right: 10px" @click.native.prevent="jumpWorkdetail(scope.row.id)" size="mini">
                  详情
                </el-button>
        ...
        <el-tab-pane v-if="istabShow('ghkh_attribution_record')" label="归属记录" name="8">
          <el-table ref="multipleTable" :data="customerAttributionRecordData" tooltip-effect="dark" style="width: 100%"
            @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55"> </el-table-column>
            ...
            <el-table-column prop="customerIndustry" label="所属行业" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="startTime" label="保护起始" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="endTime" label="保护截至"> </el-table-column>
        ...
        <el-tab-pane v-if="istabShow('ghkh_operation_log')" label="操作日志" name="9">
          <el-table ref="multipleTable" :data="operaLogData" tooltip-effect="dark" style="width: 100%"
            @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55"> </el-table-column>
    ...
    <!-- 跟进 -->
    <FollowUp ref="FollowUp" :customerId="currentCustomerId" @closeAdd="followRecordList" @reload="followRecordList"
    ...
    <!-- 新建任务 -->
    <Task ref="Task" :customerId="[currentCustomerId]" @closeAdd="workRecordList" @reload="getData" :type="0" />
    <!-- 添加联系人 -->
    <ConnectPerson :customerId="currentCustomerId" ref="ConnectPerson" :id="editConcantId" @reload="getData" />
    <!-- 添加协作 -->
    <CoopeaTion :customerId="currentCustomerId" ref="CoopeaTion" :ids="ids" :isCoop="customerInfo.collaboratorIds"
      @reload="getData" />
    <!-- 跟进详情 -->
    <FollowUpDetail ref="FollowUpDetail" @reload="getData" />
    <CustomerSea v-if="CustomerSeaopen" :id="editId" ref="CustomerSea" @reload="getData" />
  </div>
</template>
  ...
  methods: {
    whichActiveTab() {
      ...
      const Arr = [
        ...
        "ghkh_data_details",
        "ghkh_contact",
        "ghkh_lower_level_customers",
        "ghkh_follow_up_record",
        "ghkh_mission_record",
        "ghkh_work_order_record",
        "ghkh_attribution_record",
        "ghkh_operation_log",
      ];
      ...
      if (newArr.length > 0) {
        switch (newArr[0]) {
          case "ghkh_general_info":
            this.activeName = "1";
            break;
          case "ghkh_data_details":
            this.activeName = "2";
            break;
          case "ghkh_contact":
            this.activeName = "3";
            break;
          case "ghkh_lower_level_customers":
            this.activeName = "4";
            break;
          case "ghkh_follow_up_record":
            this.activeName = "5";
            break;
          case "ghkh_mission_record":
            this.activeName = "6";
            break;
          case "ghkh_work_order_record":
            this.activeName = "7";
            break;
          case "ghkh_attribution_record":
            this.activeName = "8";
            break;
          case "ghkh_operation_log":
            this.activeName = "9";
            break;
        }
      }
