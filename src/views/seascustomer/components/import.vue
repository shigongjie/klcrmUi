...
<template>
  <div>
    <el-dialog :close-on-click-modal='false' :title="'导入企业'" :visible.sync="open" width="80%">
      ...
      <div v-show="active == 2">
        ...
        <div class="textDiv">
            <p v-for="(item,index) in uploadAfterData.message" :key="index">{{item}}</p>
          </div>
...
<script>
import { seagetTemplate, seaperformImport, seaimportAll } from "@/api/customer/index";
...
export default {
  ...
  methods: {
    ...
    step2() {
      ...
      seaimportAll(formData).then((res) => {
        this.active = 3;
      });
    },
    uploadSuccess() {
    ...
    next() {
      ...
      let formData = new FormData();
      formData.append("file", this.$refs.upload.uploadFiles[0].raw);
      seaperformImport(formData).then((res) => {
        console.log(res);
