...
export default {
  ...
  beforeMount() {
    getShowList({ type: 1 }).then((res) => {
      if (sessionStorage.getItem("allcustomerColumn")) {
        this.showList = JSON.parse(sessionStorage.getItem("allcustomerColumn"));
      } else {
  ...
  methods: {
    resetColumn() {
      getShowList({ type: 1 }).then((res) => {
        this.showList = res.data;
        sessionStorage.setItem("allcustomerColumn", JSON.stringify(res.data));
    ...
    getData(query) {
      if (this.type == 2) {
        ...
        pageAll(this.queryParams).then((res) => {
          this.total = res.total;
          this.tableData = res.rows;
        });
      }
