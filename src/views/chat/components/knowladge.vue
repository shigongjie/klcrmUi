  ...
  methods: {
    ...
    removeTreeNode(node, data) {
      ...
      this.$confirm("是否确定删除对应的记录?", "是否确定删除对应的记录?", {
        ...
        .then(() => {
          classifyremove({ classifyId: data.id, dataType: 1 }).then((res) => {
            this.$message({
              type: "success",
              message: "删除成功!",
            });
    ...
    getData(data) {
      console.log(data)
      let params = { dataType: 1 };
      let newParams = Object.assign(params, data);
      contentpage(newParams).then((res) => {
        this.tableData = res.rows;
        this.total = res.total;
      });
    },
    getTreeDatas(data) {
      getTreeData({ dataType: 1 }).then((res) => {
