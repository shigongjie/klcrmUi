...
<script>
import { deleteByIds, contactexport, customerContactExport } from "@/api/customer/index";
export default {
  data() {
    return {
      concantOpen: false,
  ...
  methods: {
    ...
    exportContact(customerId) {
      customerContactExport({customerId: customerId}).then((data) => {
        if (!data) {
          this.$message.warning("文件下载失败");
          return;
        }
