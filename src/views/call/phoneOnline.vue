<template>
    <div class="container">
        <audio ref="audio" src="" id='remoteAudio' autoPlay></audio>
...
export default {
    ...
    mounted() {
        this.name = this.ob.clientName;
        this.phone = this.ob.clientPhone;
        console.log(this.ob)
        this.getSid()
    },
    methods: {
        ...
        getSid() {
          getSIPAccount().then(res => {
            ...
            setTimeout(() => {
              let data = {};
              data.taskDetailId = this.ob.id;
              data.type = 1;
              data.staffNo = this.user;
              data.callee = this.phone;
              data.calleeName = this.ob.clientName;
